...
1. **M6 — Workspace and CLI ergonomics**
2. **M7 — Managed SSH and repo-specific Git access**
3. **M8 — Host bootstrap**
...
Make `vr pi <workspace>` and `vr shell <workspace>` work safely and predictably, while also allowing Pi-specific CLI options to reach Pi without being rejected by `vr`.

### Workspace commands to support
...
Then have the Rust wrapper invoke Compose with:
...
### Pi option pass-through

#### Goal

Allow `vr` to accept and pass through Pi agent CLI options without failing argument parsing.
...
prefer an explicit -- separator for ambiguous cases.
allow direct pass-through after vr pi only when it does not conflict with workspace parsing.
```
...
Pi-specific flags can be passed through using the documented syntax.
vr --help and vr pi --help remain useful and unambiguous.
