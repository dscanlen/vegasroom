...
1. Implement the confirmed Vegasroom image tagging convention.
...
2. Harden permissions for sensitive Vegasroom state.
...
3. Add semantic config validation.
...
4. Improve config TUI information display.
...
5. Make config/runtime writes crash-safe.
...
6. Ensure all TUI menus function consistently across Vegasroom.
...
7. Update `docs/config-tui.md` to match current behavior or adjust the TUI back to the documented design.
...
8. Resolve custom Compose-file behavior.
...
9. Sanitize generated YAML values consistently.
...
10. Split `src/config_ui.rs` into smaller modules without changing behavior.
...
11. Revisit config TUI text-input editing.
...
12. Improve `vr config` Environment polish.
...
13. Revisit harness-independent package/library selection version syntax and update policy.
    - Cover `environment.apt.packages`, `environment.rust`, `environment.python`, `environment.go`, and `environment.typescript`.

14. Brainstorm dependency-cache/profile concepts before implementation.
...
15. Decide and implement release workflow details.
...
16. Decide whether pinned harness package updates should be checked on a schedule, during release preparation, or only manually.

17. Add `default_harness` / bare `vr` harness selection once multiple harnesses exist.

18. Decide whether a CLI `--color auto|always|never` override is needed in addition to persisted `ui.color` and `NO_COLOR`.

19. Revisit `vr ssh` command handling.
