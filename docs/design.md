...
- optional environment config, starting with `environment.apt.packages`, `environment.rust`, and `environment.python`, generates a derived image such as `vegasroom/pi:local-env`, which is rebuilt when the requested package/toolchain set changes
